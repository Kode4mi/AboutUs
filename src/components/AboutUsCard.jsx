...
function Card({ active, data }) {
    const [cardClasses, setCardClasses] = useState("card lg:card-side bg-slate-700 shadow-xl w-1/2 min-w-[300px] h-fit animate__animated ");
    const [cardData, setCardData] = useState({});

    useEffect(() => {
        setCardClasses("card lg:card-side bg-slate-700 shadow-xl w-1/2 min-w-[300px] h-fit animate__animated animate__fadeOutDown")
        setTimeout(() => {
    ...
    return <>
        {cardData && <div className={cardClasses}>
            ...
            <div className="card-body lg:w-2/3 h-fit ">
                ...
                <h1 className="card-title text-4xl">{cardData.name}</h1>
                <div className="text-2xl"><PortableText value={cardData.description} /></div>
